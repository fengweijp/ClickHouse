	...
	size_t aio_threshold, time_t time_of_merge, DiskSpaceMonitor::Reservation * disk_reservation)
{
	...
	for (size_t i = 0; i < parts.size(); ++i)
	{
		auto input = std::make_unique<MergeTreeBlockInputStream>(
			data, parts[i], DEFAULT_MERGE_BLOCK_SIZE, merging_column_names, MarkRanges(1, MarkRange(0, parts[i]->size)),
			...
			++column_num, ++it_name_and_type)
		{
			...
			for (size_t part_num = 0; part_num < parts.size(); ++part_num)
			{
				auto column_part_stream = std::make_shared<MergeTreeBlockInputStream>(
					data, parts[part_num], DEFAULT_MERGE_BLOCK_SIZE, column_name_, MarkRanges(1, MarkRange(0, parts[part_num]->size)),
	...
	const ReshardingJob & job, DiskSpaceMonitor::Reservation * disk_reservation)
{
	...
	for (size_t i = 0; i < parts.size(); ++i)
	{
		...
		auto input = std::make_unique<MergeTreeBlockInputStream>(
			data, parts[i], DEFAULT_MERGE_BLOCK_SIZE, column_names,
